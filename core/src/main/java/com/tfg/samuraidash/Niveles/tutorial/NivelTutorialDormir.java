...
public class NivelTutorialDormir implements Screen {
    ...
    public void show() {
        ...
        overlayTexture = new Texture(Gdx.files.internal("Overlays/4.png")); // Textura de overlay
        ...
        map = new TmxMapLoader().load("MapaTutorialDormir.tmx");  // Aquí cargamos el archivo .tmx
        ...
        samurai = new Samurai(7168, 14336); // Posición inicial del samurai
        enemigos = new Array<>();
        ...
        npcs.add(new NPC(samurai, 150, 32, "NPCS/accion.png", 6, 0.1f, Animation.PlayMode.LOOP, "Samurai", new Array<String>(new String[]{
            "Porque esta todo en llamas?",
            "Que esta pasando?"
        }), camera));
        //Añado a la cinematica las acciones, de caminar y los npcs como textos
        cine2.agregarAccion(new Caminar(samurai, 1, true));
        cine2.agregarAccion(new Dialogar(npcs.get(0)));         // Iniciar el diálogo
        ...
        cine3.agregarAccion(new Dialogar(npcs.get(1)));
        ...
        npcs.add(new NPC(samurai, 150, 32, "NPCS/accion.png", 6, 0.1f, Animation.PlayMode.LOOP, "Samurai", new Array<String>(new String[]{
            "NOOO MI CASA TIO",
            "No puedo pasar por aqui, hay un arbol",
            "Buscare otro camino"
        }), camera));
        cine4.agregarAccion(new Caminar(samurai, 0.8f, true));
        cine4.agregarAccion(new Dialogar(npcs.get(2)));
        cinematicas.add(cine4);

        //Repetimos lo anterior para otra cinematica, pero esta tiene una pantalla con texto
        ...
        textos2.add("Pase por el portal");
        textos2.add("todo se volvio negro por un momento");
        textos2.add("Que pasa, que esta pasando");
        cine5.agregarAccion(new PantallaNegraConTexto(2f, textos2));
        Game game = (Game) Gdx.app.getApplicationListener();
        NivelTutorialFinal nuevoScreen = new NivelTutorialFinal();
        cine5.agregarAccion(new CambiarPantalla(game, nuevoScreen));
        ...
        // Cargamos sonidos
        musicaFondo = Gdx.audio.newMusic(Gdx.files.internal("musica/tutorial2.mp3"));
    ...
    public void render(float delta) {
        ...
        if (!menuPausa.isVisible()) {
            if (!cinematicas.get(3).isEnCinematica()) {
                musicaFondo.play();
            }
        ...
        for (MapObject object : objects) {
            if (object instanceof RectangleMapObject) {
                ...
                if (samurai.getBounds().overlaps(rect)) {
                    ...
                    if (name.equals("cinematica4") && !cinematicas.get(1).isEnCinematica()) {
                        // Inicia la cinematica
                        cinematicas.get(1).iniciar();
                    } else if (name.equals("cinematica1") && !cinematicas.get(2).isEnCinematica()) {
                        // Inicia la cinematica
                        cinematicas.get(2).iniciar();
                    } else if (name.equals("cinematica3") && !cinematicas.get(3).isEnCinematica()) {
                        // Inicia la cinematica
