...
public class NivelTutorialFinal implements Screen {
    ...
    public void show() {
        ...
        overlayTexture = new Texture(Gdx.files.internal("Overlays/9.png")); // Textura de overlay
        font = new BitmapFont(Gdx.files.internal("fonts/fuente.fnt")); // Fuente para texto
        backgroundTexture = new Texture("Fondos/nature_6/1.png"); // Fondo del nivel
        ...
        map = new TmxMapLoader().load("MapaTutorialFinal.tmx");  // Aquí cargamos el archivo .tmx
        ...
        samurai = new Samurai(352, 5258); // Posición inicial del samurai
        ...
        npcs.add(new NPC(samurai, 150, 32, "NPCS/accion.png", 6, 0.1f, Animation.PlayMode.LOOP, "Jefe Ronin", new Array<String>(new String[]{
            "D-detente! No tienes idea de con quien te metes, samurai!"
        }), camera));
        npcs.add(new NPC(samurai, 150, 32, "NPCS/accion.png", 6, 0.1f, Animation.PlayMode.LOOP, "Samurai", new Array<String>(new String[]{
            "Quien sois? Por que atacasteis mi hogar?"
        }), camera));
        npcs.add(new NPC(samurai, 150, 32, "NPCS/accion.png", 6, 0.1f, Animation.PlayMode.LOOP, "Jefe Ronin", new Array<String>(new String[]{
            "Nosotros no... no tuvimos eleccion! Ellos... los del otro lado... nos prometieron poder."
        }), camera));
        npcs.add(new NPC(samurai, 150, 32, "NPCS/accion.png", 6, 0.1f, Animation.PlayMode.LOOP, "Samurai", new Array<String>(new String[]{
            "Quienes son \"ellos\"? Que buscan?"
        }), camera));
        npcs.add(new NPC(samurai, 150, 32, "NPCS/accion.png", 6, 0.1f, Animation.PlayMode.LOOP, "Jefe Ronin", new Array<String>(new String[]{
            "Cientificos... o brujos, no lo se! Vinieron de ese portal hablando de guerras"
        }), camera));
        npcs.add(new NPC(samurai, 150, 32, "NPCS/accion.png", 6, 0.1f, Animation.PlayMode.LOOP, "Jefe Ronin", new Array<String>(new String[]{
            "De recursos que ya no existen en su tiempo. Nos usaron para saquear, para preparar algo."
        }), camera));
        npcs.add(new NPC(samurai, 150, 32, "NPCS/accion.png", 6, 0.1f, Animation.PlayMode.LOOP, "Jefe Ronin", new Array<String>(new String[]{
            "Pero algo salio mal! El portal empezo a traer cosas... cosas que no deberian existir!"
        }), camera));
        npcs.add(new NPC(samurai, 150, 32, "NPCS/accion.png", 6, 0.1f, Animation.PlayMode.LOOP, "Samurai", new Array<String>(new String[]{
            "Entonces, mi pueblo... fue solo un paso en vuestra locura."
        }), camera));
        npcs.add(new NPC(samurai, 150, 32, "NPCS/accion.png", 6, 0.1f, Animation.PlayMode.LOOP, "Jefe Ronin", new Array<String>(new String[]{
            "Yo solo queria sobrevivir! Todos estamos atrapados en esto!"
        }), camera));
        npcs.add(new NPC(samurai, 150, 32, "NPCS/accion.png", 6, 0.1f, Animation.PlayMode.LOOP, "Jefe Ronin", new Array<String>(new String[]{
            "Si cruzas ese portal, entenderas! Ninguno de nosotros es el verdadero enemigo!"
        }), camera));
        npcs.add(new NPC(samurai, 150, 32, "NPCS/accion.png", 6, 0.1f, Animation.PlayMode.LOOP, "Narrador", new Array<String>(new String[]{
            "(El samurai lo observa en silencio, luego mira fijamente el portal)"
        }), camera));
        npcs.add(new NPC(samurai, 150, 32, "NPCS/accion.png", 6, 0.1f, Animation.PlayMode.LOOP, "Narrador", new Array<String>(new String[]{
            "(El jefe ronin cae de rodillas, sin fuerzas para seguir luchando.)"
        }), camera));
        npcs.add(new NPC(samurai, 150, 32, "NPCS/accion.png", 6, 0.1f, Animation.PlayMode.LOOP, "Samurai", new Array<String>(new String[]{
            "Si es ahi donde esta la respuesta, entonces no me detendre."
        }), camera));
        //Añado a la cinematica las acciones, de caminar y los npcs como textos
        cine2.agregarAccion(new Caminar(samurai, 0.4f, false));
        ...
        // Cargamos enemigos y NPCs
        loadEnemigos("niveles/Tutorial/NivelTutorial_Enemigos.txt");
        loadNPCs("niveles/Tutorial/NivelTutorial_NPC2.txt");
        ...
        // Cargamos sonidos
        musicaFondo = Gdx.audio.newMusic(Gdx.files.internal("musica/tutorial3.mp3"));
    ...
    public void render(float delta) {
        ...
        if (!menuPausa.isVisible()) {
            ...
            if (isDeathTransition) {
                ...
                if (isTransitionIn) {
                    ...
                    if (transitionTimer >= transitionDuration) {
                        ...
                        transitionTimer = 0;
                        // Aquí recargo los enemigos mientras la pantalla está completamente negra
                        enemigos.clear();
                        loadEnemigosAsync();
                    }
            ...
            samurai.update(delta, plataformas, enemigos); // Ahora pasamos las plataformas como colisiones
            ...
            if(!samurai.isDead() && !samurai.isFrenzyActive() && !samurai.isInDialogue() && !cinematicas.get(0).isEnCinematica()) {
    ...
    private void loadEnemigosAsync() {
        new Thread(() -> {
            ...
            FileHandle file = Gdx.files.internal("niveles/Tutorial/NivelTutorial_Enemigos.txt");
        ...
        for (MapObject object : objects) {
            if (object instanceof RectangleMapObject) {
                ...
                if (samurai.getBounds().overlaps(rect)) {
                    ...
                    if (name.equals("Muerte")) {
                        // El samurai muere
                        samurai.die();
                    } else if (name.equals("boss") && cine == 0 && !cinematicas.get(0).isEnCinematica()) {
    ...
    private void saveGameProgress(boolean tutorialCompletado, int nivelMaxCompletado) {
        ...
        if (gameData == null) {
            gameData = new Datos("Partida " + (selectedSlot + 1), nivelMaxCompletado, tutorialCompletado);
        } else {
            gameData.setTutorialCompletado(tutorialCompletado);
